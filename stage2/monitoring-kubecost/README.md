...
```mermaid
flowchart TB
    UserBrowser["User Browser"]:::edge
    NginxIngress["NGINX Ingress"]:::edge
    OAuthProxy["OAuth2 Proxy"]:::edge

    subgraph nskubecost [Namespace: kubecost]
        FrontendUI["frontend<br/>nginx :9090"]:::app
        Aggregator["aggregator<br/>StatefulSet + ClickHouse"]:::app
        CloudCost["cloud-cost"]:::app
        Forecasting["forecasting"]:::app
        LocalStore["local-store<br/>cluster-storage :9006"]:::app
        FinOpsAgent["finops-agent<br/>cluster scraper"]:::app
    end

    MinioBucket["MinIO<br/>kubecost-federated-store"]:::store
    ...
    classDef store fill:#1e6b52,color:#ffffff
```
