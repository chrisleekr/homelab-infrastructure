...
```mermaid
flowchart LR
    CIPipeline["CI pipeline<br/>pushes image:master"]:::aux
    Registry["Container registry<br/>registry.chrislee.local"]:::aux
    Updater["ArgoCD Image Updater<br/>this module"]
    GitOpsRepo["argocd-apps repo<br/>kustomization.yaml"]:::aux
    ArgoCD["ArgoCD"]:::aux
    Workload["Workload pod"]:::aux
