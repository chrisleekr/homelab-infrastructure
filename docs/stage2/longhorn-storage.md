...
```mermaid
flowchart TB
    PodApp["Pod with Longhorn PVC<br/>scheduled on a worker"]
    CSIWorker["Worker: longhorn-manager + csi-plugin<br/>tolerates node.homelab/class<br/>no disk, no replicas"]
    StorageCP["Control plane: labeled create-default-disk=true<br/>holds the disk and every replica"]
