...
```mermaid
flowchart TD
    PRE["Preflight on control plane<br/>skew, health, etcd + PKI backup, removed APIs"]
    CIL["Upgrade Cilium<br/>dry-run, then agent upgrade"]
    CP["Upgrade control plane<br/>kubeadm binary, upgrade plan, upgrade apply, kubelet"]
    WRK["Per worker, one at a time<br/>kubeadm binary, upgrade node, drain, runtime + kubelet, wait Ready, uncordon"]
    POST["Postflight<br/>every node Ready on the target version"]
    PRE --> CIL --> CP --> WRK --> POST
```
