...
!!! info "Why `serial: 1` here"
    ...
    Parallel host forks collide on the shared AnsiballZ module cache ([ansible/ansible#16489](https://github.com/ansible/ansible/issues/16489)), so any play with more than one host needs `serial: 1`.
...
Runs [`kubeadm_pre_setup`](roles/kubeadm-pre-setup.md) then [`kubeadm_agent`](roles/kubeadm-agent.md), kubeadm only. Skipped entirely when `agent` is empty ("skipping: no hosts matched").
